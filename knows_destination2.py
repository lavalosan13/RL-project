...
# ==============================
class Drone:
    def __init__(self, env, eps=0.5):  # Increase eps to enlarge the detection zone
        self.position = np.zeros(3, dtype=np.float32)
        self.velocity = np.zeros(3, dtype=np.float32)
        self.acceleration = np.zeros(3, dtype=np.float32)
        self.battery = 10000.0
        self.env = env
        self.eps = eps  # Add eps parameter
    ...
    def move(self, action):
        ...
        action = np.clip(action, -1, 1)  # Clip the action to be within the range [-1, 1]
        action = action / np.linalg.norm(action)  # Normalize the action to have a norm of 1
        self.acceleration = action * 0.01  # Scale the action to control the speed
...
class DroneEnv(gym.Env):
    ...
    def step(self, action=None):
        ...
        hit_obstacle = any(np.linalg.norm(self.drone.position - obs_center) <= self.drone.eps for obs_center, _ in self.env.obstacles)
        battery_depleted = self.drone.battery <= 0

        done = reached_destination or hit_obstacle or battery_depleted
        reward = self.compute_reward(reached_destination, hit_obstacle, battery_depleted)
        return self._get_obs(), reward, done, {}

    def compute_reward(self, reached_destination, hit_obstacle, battery_depleted):
        if reached_destination:
            return 100000
        elif hit_obstacle or battery_depleted:
            return -50000
