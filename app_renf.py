...
class Drone:
    ...
    def reset(self, env):
        self.position = np.array([env.size/2, env.size/2, env.size/2], dtype=np.float32)
        self.velocity = np.zeros(3)
        self.acceleration = np.zeros(3)
        self.battery = 100.0
        self.env = env
...
def train(env, model, optimizer, epochs=1, gamma=0.99, reward_threshold=900, window_size=100):
    ...
    for episode in range(epochs):
        ...
        while not done:
            ...
            action_probs, state_value = model(state_tensor)                  #obtenir les probabilités d'action et la valeur de l'état
            
            action = torch.multinomial(action_probs, 1).item()              #choisir une action selon les probabilités
            new_state, reward, done, _ = env.step(action)               #faire l'action et obtenir le nouvel état
            print(action_probs)                             #afficher les probabilités d'actions
            log_probs.append(torch.log(action_probs[0, action]).unsqueeze(0))           # Ensure log_probs are 1D tensors
            values.append(state_value)                  #ajouter la valeur de l'état
            rewards.append(reward)                  #ajouter la récompense
            
            state = new_state #mettre à jour l'état

        # Calcul des pertes Actor-Critic
        returns, G = [], 0 #initialiser les retours et G
        for r in reversed(rewards):
            G = r + gamma * G #calculer le retour
            returns.insert(0, G) #ajouter le retour à la liste des retours

        returns = torch.tensor(returns, dtype=torch.float32)  # Ensure returns are Float tensors
        values = torch.cat(values).squeeze() #concaténer les valeurs et les aplatir
        advantage = returns - values.detach() #calculer l'avantage
        print(f'logprob is {log_probs}') #afficher les logprobs
        print(f'advantage is {advantage}') #afficher l'avantage

        if log_probs:  # Ensure log_probs is not empty
            actor_loss = -(torch.cat(log_probs) * advantage).mean() #calculer la perte de l'acteur
...
def test(env, model, max_steps=1000):
    ...
    while not done and steps < max_steps:
        ...
        state, reward, done, _ = env.step(action)
        #state, reward, done, _ = env.step() # pour tester avec une action aléatoire
        total_reward += reward
        steps += 1
        trajectory.append(env.drone.position.copy())  # Enregistrer la position actuelle du drone
